...
def format_anamnezis_csv(gen_csv: pd.DataFrame):
    ...
    gen_csv = gen_csv.groupby(['Kezdete','BNO-10', 'Forrás(ok)','BNO leírás', 'Bejegyzés dátuma','Rendezési dátum'], as_index=False, dropna=False).agg(lambda x: ', '.join(x))
    gen_csv = gen_csv[[gen_csv.columns[6],gen_csv.columns[0],gen_csv.columns[4],gen_csv.columns[1],gen_csv.columns[3],gen_csv.columns[2],gen_csv.columns[5]]]
